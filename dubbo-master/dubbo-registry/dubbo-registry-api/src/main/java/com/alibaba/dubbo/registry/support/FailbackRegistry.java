...
public abstract class FailbackRegistry extends AbstractRegistry {
    ...
    /***
     * 按周期重试失败的记录，（failedRegistered，failedUnregistered， failedSubscribed， failedUnsubscribed， failedNotified）
     * @param url
     */
    public FailbackRegistry(URL url) {
        super(url);
        int retryPeriod = url.getParameter(Constants.REGISTRY_RETRY_PERIOD_KEY, Constants.DEFAULT_REGISTRY_RETRY_PERIOD);
        /** 1要执行的任务,2首次执行的延迟时间,3一次执行终止和下一次执行开始之间的延迟,4时间单位 */
    ...
    /// NotifyListener : 服务端：@RegistryProtocol.OverrideListener  客户端：@RegistryDirectory
    @Override
    public void subscribe(URL url, NotifyListener listener) {
        super.subscribe(url, listener);///listener = RegistryDirectory
